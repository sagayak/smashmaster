    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-white/50 backdrop-blur-xl p-6 rounded-[2.5rem] border border-white/60 shadow-xl">
              ...
              <div className="relative">
                ...
                {isEditingFormat && isAdmin && (
                  <div className="absolute top-full mt-2 left-0 bg-white/95 backdrop-blur-md border border-slate-200 rounded-2xl shadow-2xl p-2 z-50 min-w-[200px] animate-in slide-in-from-top-2">
                    <button onClick={() => updateFormat('League')} className="w-full text-left px-4 py-3 hover:bg-indigo-50 rounded-xl text-sm font-bold text-slate-700 transition-colors flex items-center justify-between">League Table {tournament?.format === 'League' && <Check className="w-4 h-4 text-emerald-500" />}</button>
                    <button onClick={() => updateFormat('Knockout')} className="w-full text-left px-4 py-3 hover:bg-indigo-50 rounded-xl text-sm font-bold text-slate-700 transition-colors flex items-center justify-between">Knockout Bracket {tournament?.format === 'Knockout' && <Check className="w-4 h-4 text-emerald-500" />}</button>
                  </div>
                )}
              </div>
              {isAdmin && (
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <button onClick={handleShare} className="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-white/40 border border-white/60 px-4 py-2.5 rounded-2xl text-slate-700 font-bold hover:bg-white/60 transition-all group">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {currentHandbook.slice(0, 4).map((section, idx) => (
                <div key={section.id || idx} className="flex gap-4">
                  <div className="shrink-0 w-8 h-8 rounded-xl bg-indigo-50 flex items-center justify-center border border-indigo-100"><div className="scale-75">{ICON_MAP[section.iconName] || <CheckCircle2 className="w-4 h-4 text-indigo-600" />}</div></div>
                  <div><h4 className="text-[11px] font-black text-slate-900 uppercase tracking-tight">{section.title}</h4><p className="text-[10px] text-slate-500 font-bold leading-relaxed line-clamp-2">{section.content}</p></div>
                </div>
              ))}
            </div>
            <button onClick={handleOpenHandbook} className="mt-8 w-full flex items-center justify-center gap-2 text-indigo-700 font-black uppercase tracking-widest text-[10px] hover:gap-4 transition-all py-4 bg-slate-50/50 rounded-2xl border border-slate-100/50">Open Comprehensive Guide <ChevronRight className="w-4 h-4" /></button>
          </section>

          <section>
            <div className="flex items-center justify-between mb-4 px-2">
              <h3 className="text-lg font-black text-slate-900 flex items-center gap-2 uppercase tracking-tight"><span className="flex h-2.5 w-2.5 rounded-full bg-red-500 animate-pulse"></span> On Court Now</h3>
            </div>
